...
import { buildPublicCourseworkVerificationFetchUrl } from "@/utils/courseworkVerificationUrl";
...
const STATUS_LABEL: Record<string, string> = {
    pending: "still under faculty review",
    revision_requested: "sent back for a revision",
    rejected: "not approved",
};

function pendingMessage(body: VerifyJson | null): string {
    const status = (body?.status || "").trim();
    const label = STATUS_LABEL[status] || "not yet approved";
    return `This coursework record is on file, but is ${label}.`;
}

export default function CourseworkVerificationClient({ verificationKey }: Props) {
    ...
    useEffect(() => {
        let cancelled = false;
        (async () => {
            const targetUrl = buildPublicCourseworkVerificationFetchUrl(verificationKey);
            if (!targetUrl) {
                if (!cancelled) {
                    setState("error");
            ...
            <div className="mx-auto max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
                ...
                {state === "loading" ? (
                    ...
                    <div className="space-y-4 text-center">
                        ...
                        <h1 className="text-xl font-bold tracking-tight text-slate-900">Verified coursework badge</h1>
                        <p className="text-sm font-medium text-slate-500">
                            This CIEL coursework project is on file as faculty-approved.
                        </p>
                        {body?.project_title ? (
                        ...
                        {body?.rank != null && body?.of != null ? (
                            <p className="text-sm font-semibold text-slate-700">
                                Ranked #{body.rank} of {body.of}
                                {body.scope ? ` · ${body.scope}` : ""}
                                {body.badge_level ? ` · ${body.badge_level}` : ""}
                            </p>
                        ) : null}
                        {body?.verification_code ? (
                            <p className="text-xs font-mono font-medium text-slate-400">{body.verification_code}</p>
                        ) : null}
                        {body?.verified_at ? (
                    ...
                    <div className="space-y-4 text-center">
                        ...
                        <p className="text-sm font-medium text-slate-500">
                            {state === "not_found"
                                ? body?.message || "This verification link does not match any coursework record on file."
                                : state === "not_verified"
                                  ? pendingMessage(body)
