...
const DEFAULT_CATEGORIES: CategoryOption[] = [
    ...
    { value: "opportunity", label: "Opportunities & applications" },
    { value: "project", label: "Projects & attendance" },
...
export type PortalHelpPageProps = {
    /** e.g. `/api/v1/faculty/support` */
    apiBase: string;
...
export default function PortalHelpPage({
    ...
    submitBlurb = "Describe your issue clearly. Our team or your institution's admin may respond depending on category.",
}: PortalHelpPageProps) {
    ...
    const loadFaqs = useCallback(async () => {
        try {
            const res = await authenticatedFetch(`${apiBase}/faqs`, {}, { redirectToLogin: false });
            if (!res?.ok) return;
    ...
    const loadTickets = useCallback(async () => {
        ...
        try {
            const res = await authenticatedFetch(`${apiBase}/tickets`, {}, { redirectToLogin: false });
            if (!res?.ok) {
                setTickets([]);
                return;
            }
    ...
    const trackTicket = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const res = await authenticatedFetch(`${apiBase}/tickets/${encoded}`, {}, { redirectToLogin: false });
            if (res?.ok) {
                const body = await res.json();
            ...
            const res2 = await authenticatedFetch(
                `${apiBase}/tickets/track?reference=${encoded}`,
                {},
            ...
            toast.message("Ticket not found", {
                description: "Check the ID or reference, or open My tickets.",
            });
        } catch {
            ...
            <div className="mx-auto max-w-6xl">
                ...
                {tab === "faqs" && (
                    <div className="space-y-3">
                        {faqs.length === 0 ? (
                            <p className="rounded-xl border border-slate-200/80 bg-white px-5 py-8 text-center text-sm text-slate-500 shadow-sm">
                                No FAQs published yet. Submit a ticket or email support@cielpk.com.
                            </p>
                        ) : (
                            faqs.map((item) => (
                                <details
                        ...
                        {ticketsLoading ? (
                            ...
                            <div className="rounded-2xl border border-slate-200/80 bg-white px-6 py-16 text-center shadow-sm">
                                ...
                                <p className="mt-2 text-sm text-slate-500">
                                    Your support requests will appear here. You can also email{" "}
                ...
                {tab === "submit" && (
                    ...
                    >
                        ...
                        <p className="mt-1 text-sm text-slate-500">{submitBlurb}</p>
                    ...
                    <div className="space-y-6">
                        ...
                        >
                            ...
                            <p className="mt-1 text-sm text-slate-500">
                                Enter the ticket number or reference you received after submitting.
                            </p>
                            <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-end">
                                <div className="min-w-0 flex-1">
                                    ...
                                    <input
                                        ...
                                        className="mt-2 w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-900 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20"
                                        placeholder="e.g. FC-A1B2C3D4 or 42"
                                        autoComplete="off"
                                    />
                                </div>
                                <button
