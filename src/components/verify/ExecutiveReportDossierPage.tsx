...
import { formatSdgGoalPadded, mergeReportSdgSnapshotRows } from '@/app/dashboard/student/report/utils/reportSdgMerge';
...
export interface ExecutiveReportDossierConfig {
    reportApiPath: (reportId: string) => string;
    verifyApiPath?: (reportId: string) => string;
    backHref: string;
    backLabel: string;
    badges: string[];
    afterVerifyRedirect?: string;
    readOnlyAudience?: "faculty" | "partner";
    notFoundMessage?: string;
}

export function ExecutiveReportDossierPage({ config }: { config: ExecutiveReportDossierConfig }) {
    ...
    const fetchReportDetail = async () => {
        ...
        try {
            setLoading(true);
            const response = await authenticatedFetch(config.reportApiPath(reportId));
            if (response?.ok) {
                const data = await response.json();
                const raw = data.data || data.report || data;
                const prepared = prepareReportForVerifyDossier(raw as Record<string, unknown>);
                setReport(prepared as unknown as ReportDetail);
                setQualityAlerts(checkReportQuality(prepared));
            } else {
                toast.error(
                    config.readOnlyAudience === "faculty"
                        ? "Report not available yet (admin approval may be pending)"
    ...
    const handleVerify = async (action: "approve" | "reject") => {
        if (!config.verifyApiPath) return;
        ...
        try {
            ...
            const response = await authenticatedFetch(config.verifyApiPath(String(params.reportId)), {
                method: "PATCH",
            ...
            if (response?.ok) {
                ...
                setTimeout(
                    () => router.push(config.afterVerifyRedirect || config.backHref),
                    1500,
                );
            } else {
    ...
    const section1FacultyEmails = readSection1TeamFacultyEmails(
        report?.section1 as Record<string, unknown> | undefined,
    );
            ...
            <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
                <div className="max-w-md text-center">
                    <h2 className="text-2xl font-bold text-slate-900 mb-2">
                        {config.notFoundMessage || "Report not found"}
                    </h2>
                    <button
                        type="button"
    ...
    const canSubmitDecision = Boolean(config.verifyApiPath) && partnerCanSubmitDecision(report);
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-slate-50 p-0 sm:p-6 lg:p-8">
            <div className="max-w-7xl mx-auto space-y-6">
                ...
                <div className="flex flex-col items-stretch justify-between gap-3 sm:flex-row sm:items-center">
                    ...
                    <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                        {config.badges.map((badge) => (
                            <span
                                key={badge}
                                className="px-4 py-2 bg-indigo-50 text-indigo-700 rounded-xl text-xs font-black border border-indigo-200 uppercase tracking-widest"
                            >
                                {badge}
                            </span>
                        ))}
                ...
                {config.readOnlyAudience === "faculty" && (
                    <div className="bg-slate-50 border border-slate-200 rounded-3xl p-6 mt-8 mb-16 text-slate-700 text-sm font-medium">
                        View-only executive dossier for your supervised students. Approval and verification actions are handled by CIEL Admin and partner organizations.
                    </div>
                )}

                {!canSubmitDecision &&
                    config.readOnlyAudience !== "faculty" &&
                    normalizeKey(report.partner_status) === "approved" && (
