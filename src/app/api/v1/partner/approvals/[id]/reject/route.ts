...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        const response = await fetch(`${backendBase}/partner/approvals/${encodeURIComponent(id)}/reject`, {
            method: "POST",
            headers: {
        ...
        if (!response.ok) {
            return NextResponse.json(
                {
                    success: false,
                    message: (data as { message?: string }).message || "Failed to reject",
        ...
        console.error("partner/approvals reject proxy:", error);
