...
// ---------------------------------------------------------------------------
// Types (mirror the backend's CourseProjectEntry groups)
// ---------------------------------------------------------------------------
...
interface CourseProjectEntry {
    ...
    aimsInfo: AimsInfo | null;
    processInfo: ProcessInfo | null;
    resultsInfo: ResultsInfo | null;
    sdgMapping: SdgMapping | null;
    reflectionInfo: ReflectionInfo | null;
    moduleInclusion: ModuleInclusion | null;
    sectionSummaries: SectionSummaries | null;
    addedNote: string | null;
    stepCompleted: number;
    status: "draft" | "submitted";
}

const EMPTY: CourseProjectEntry = {
    course: "",
    projectTitle: "",
    projectDescription: "",
    sdgs: [],
...
const FORMATS: Record<string, { p: ModuleInclusion; note: string }> = {
...
const INC_TILES: { key: keyof ModuleInclusion; emoji: string; label: string }[] = [
...
export default function CourseProjectPage() {
    ...
    const mergeEntry = (base: CourseProjectEntry, data: Partial<CourseProjectEntry>): CourseProjectEntry => ({
        ...base,
        ...data,
    ...
    const inc: ModuleInclusion = entry.moduleInclusion || {};
    ...
    const save = async (patch: Partial<CourseProjectEntry>, advanceTo?: number) => {
        ...
        try {
            ...
            setEntry((e) => mergeEntry(e, result.data as Partial<CourseProjectEntry>));
    ...
    // -------- Live composed summaries (student's own words, not a generic template) --------
    ...
    const sectionKeys: (keyof SectionSummaries)[] = ["course", "assignment", "aims", "process", "results", "sdg", "reflection"];
    const activeStepIdx: number[] = [0, 1];
    if (inc.aim) activeStepIdx.push(2);
    if (inc.act || inc.meth) activeStepIdx.push(3);
    activeStepIdx.push(4, 5, 6);
                    ...
                    {step === 7 && (
                        <div className="space-y-5">
                            ...
                            <div className="space-y-3">
                                ...
                                <p className="text-xs text-ciel-text-soft">Gathered from your {activeStepIdx.length} applicable sections — built from your own answers, not a generic template.</p>
                                {activeStepIdx.map((i, order) => {
                                    const stepDef = STEPS[i];
                                    const key = sectionKeys[order];
                                    const text = sums[key];
                                    return (
                                        <div key={i} className="rounded-ciel-sm border-2 border-ciel-border bg-ciel-page/50 p-4">
                                            <p className="text-xs font-black uppercase tracking-widest text-ciel-text-soft">{stepDef.emoji} {stepDef.label}</p>
                                            <p className="mt-1.5 text-sm leading-relaxed text-ciel-text">{text || <span className="text-ciel-text-soft">Nothing captured — go back and fill it in.</span>}</p>
                                        </div>
                                    );
                                })}
                            </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        {entry.status === "submitted" ? (
                            <button
                                type="button"
                                disabled={saving}
                                onClick={() => save(saveAllFields())}
                                className="ciel-transition rounded-ciel-sm border border-ciel-green/40 bg-ciel-green-soft px-5 py-2.5 text-sm font-bold text-ciel-green-deep hover:bg-ciel-green-soft/80 disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ciel-green"
                            >
                        ...
                        {step < 7 ? (
                            ...
                            <span className="rounded-ciel-sm bg-ciel-green-soft px-4 py-2.5 text-sm font-bold text-ciel-green-deep">
                                Submitted — admin can review your latest save
                            </span>
                        ) : (
                            <button
                                type="button"
                                disabled={saving}
                                onClick={() => save({ ...saveAllFields(), status: "submitted", sectionSummaries: sums }, 8)}
