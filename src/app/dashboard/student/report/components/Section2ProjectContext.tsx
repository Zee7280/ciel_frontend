...
export default function Section2ProjectContext({ projectData }: Section2Props) {
    ...
    const handleGenerateAISummary = async () => {
        if (!sectionData.problem_statement) {
            toast.error("Please provide a problem statement first");
            return;
        }

        setIsGenerating(true);
        const result = await generateAISummary("section2", sectionData);
        setIsGenerating(false);

        if (result.error) {
            toast.error(result.error);
        } else if (result.summary) {
            updateSection('section2', { summary_text: result.summary });
    ...
    const debouncedProblemStatement = useDebounce(sectionData.problem_statement, 3000);
    const [lastAutoGeneratedFor, setLastAutoGeneratedFor] = useState('');

    useEffect(() => {
        const canAutoGenerate =
            debouncedProblemStatement?.length > 300 && // Roughly 50-60 words
            debouncedProblemStatement !== lastAutoGeneratedFor &&
            (!sectionData.summary_text ||
                sectionData.summary_text.includes("This project addresses a documented gap") ||
                sectionData.summary_text.length < 50);

        if (canAutoGenerate && !isGenerating && !isReadOnly) {
            setLastAutoGeneratedFor(debouncedProblemStatement);
            handleGenerateAISummary();
        }
    }, [debouncedProblemStatement, sectionData.summary_text, isGenerating, isReadOnly]);
                        ...
                        <div className="space-y-1">
                            <div className="flex items-center justify-between mb-2">
                                ...
                                <button
                                    ...
                                    disabled={isGenerating || !sectionData.problem_statement}
                                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-report-primary text-white hover:bg-report-primary/90 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-md shadow-report-primary-shadow group"
                                >
                                    {isGenerating ? (
                                    ...
                                    <span className="text-[10px] font-black uppercase tracking-wider">Improve with AI</span>
                                </button>
                            </div>
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        <button
                            ...
                            onClick={handleGenerateAISummary}
                            disabled={isGenerating || !sectionData.problem_statement}
                            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-report-primary text-white text-[10px] font-black uppercase tracking-widest hover:bg-report-primary/90 disabled:opacity-50 transition-all shadow-lg shadow-report-primary-shadow group"
                        >
                            {isGenerating ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Sparkles className="w-3.5 h-3.5 group-hover:rotate-12 transition-transform" />}
