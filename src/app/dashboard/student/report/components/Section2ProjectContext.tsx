...
const GAP_OPTIONS = ["Skills", "Access", "Resources", "Systems", "Awareness"];
...
export default function Section2ProjectContext({ projectData }: Section2Props) {
    ...
    const composeBaseline = (
        ...
        if (gaps.length) {
            parts.push(`The core gaps were in ${listJoin(gaps.map((g) => g.toLowerCase()))}.`);
        }
    ...
    // ─── Auto-compose the baseline statement as answers come in ───────────────
    React.useEffect(() => {
        const words = (sectionData.problem_statement || "").trim().split(/\s+/).filter((w) => w).length;
