...
export default function Section3SDGMapping({ projectData }: Section3Props) {
    ...
    const handleGenerateAISummary = async () => {
        if (!contribution_intent_statement) {
            toast.error("Please provide a contribution intent statement first");
            return;
        }

        setIsGenerating(true);
        const result = await generateAISummary("section3", section3);
        setIsGenerating(false);

        if (result.error) {
            toast.error(result.error);
        } else if (result.summary) {
            updateSection('section3', { summary_text: result.summary });
    ...
    const debouncedIntent = useDebounce(contribution_intent_statement, 3000);
    const [lastAutoGeneratedFor, setLastAutoGeneratedFor] = useState('');

    useEffect(() => {
        const canAutoGenerate =
            debouncedIntent?.length > 200 &&
            debouncedIntent !== lastAutoGeneratedFor &&
            (!section3.summary_text ||
                section3.summary_text.includes("This project is aligned with SDG") ||
                section3.summary_text.length < 50);

        if (canAutoGenerate && !isGenerating) {
            setLastAutoGeneratedFor(debouncedIntent);
            handleGenerateAISummary();
        }
    }, [debouncedIntent, section3.summary_text, isGenerating]);
                            ...
                            <div className="flex items-start gap-3 p-3 rounded-xl border" style={{ borderColor: sdg.color + '40', backgroundColor: sdg.color + '08' }}>
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2 flex-wrap">
                                        <p className="report-h3 !text-xs !normal-case tracking-normal">SDG {sdg.num}: {sdg.name}</p>
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                        <button
                            ...
                            disabled={isGenerating || !contribution_intent_statement}
                            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-report-primary text-white text-[10px] font-black uppercase tracking-widest hover:bg-report-primary/90 disabled:opacity-50 transition-all shadow-lg shadow-report-primary-shadow"
                        >
                            {isGenerating ? <Loader2 className="w-3 h-3 animate-spin" /> : <Sparkles className="w-3 h-3 text-white" />}
