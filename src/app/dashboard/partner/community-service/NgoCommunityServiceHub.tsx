...
const GUIDES: Record<string, { title: string; desc: string; items?: [string, string][]; rule: string }> = {
    ...
    files: {
        ...
        desc: "Faculty Analysis files and AI Analyzer reports shared with every stakeholder on the record — same file, same version, every dashboard.",
        rule: "You see the same file the student, faculty and CIEL PK see.",
    },
    run: {
...
export default function NgoCommunityServiceHub() {
    ...
    const createTab = (["drafts", "review", "action", "published", "closed"].includes(innerTab) ? innerTab : defaultCreateTab) as
        ...
        | "published"
        | "closed";
    const approvalTab = ["pending", "done"].includes(innerTab) ? innerTab : "pending";
                        ...
                        {loading ? (
                            ...
                            <div className="mt-3 rounded-[14px] border border-[#dde8e6] bg-white px-3 py-6 text-center text-sm text-slate-600">
                                ...
                                <p className="mt-1 text-[12px] text-slate-500">
                                    New Flashcards appear here when a student or faculty names your NGO.
                                </p>
                            </div>
                        ) : (
            ...
            {(view === "approvals" || view === "pending") && (
                <div className="mt-4">
                    ...
                    {loading ? (
                        ...
                        (() => {
                            const list = (view === "pending" ? "pending" : approvalTab) === "done" ? decidedApprovals : pendingApprovals;
                            if (!list.length) {
                                return (
                                    <EmptyPanel
                                        title={approvalTab === "done" ? "No decided records yet" : "No approvals waiting"}
                                        text="When an opportunity lists your organisation as partner and your review step is due, it will appear here."
                                    />
                                );
                            }
                            return (
                                <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                                    {list.map((row) => (
                                        <CommunityQueueCard
                                            key={row.id}
                                            href={`${APPROVALS}?tab=${(view === "pending" ? "pending" : approvalTab) === "done" ? "history" : "pending"}&opportunity=${encodeURIComponent(row.id)}`}
                                            title={row.title}
                                            student={pickStr(row, "creator_name", "student_name", "submitted_by_name") || "Creator"}
                                            cta={approvalTab === "done" ? "Open record →" : "View Flashcard & Approve →"}
                                            tone={approvalTab === "done" ? "approved" : "waiting"}
                                        />
                                    ))}
                                </div>
                            );
